...
//    }
    ...
    @Transactional
    public ResponseDto<?> addLikesMovieUpComming(Long id, HttpServletRequest request) {
        ...
        // 좋아요 한번만 체크
        if (likesRepository.existsByMember_IdAndMovieUpComming_id(member.getId(), movieUpComming.getId())) {

            likesRepository.deleteByMember_IdAndMovieUpComming_Id(member.getId(), movieUpComming.getId());

            Integer postlike_count = likesRepository.countAllByMovieUpComming(movieUpComming);
            // 포스트 업데이트
            movieUpComming.updatelike_count(postlike_count);
    ...
    public ResponseDto<?> addLikesComment(Long id, HttpServletRequest request) {
        ...
        if (likesRepository.existsByMember_IdAndComment_Id(member.getId(), comment.getId())) {
            ...
            return ResponseDto.fail("NickName_Is_Already exist",
                    "이미 좋아요를 누르셨습니다.");
